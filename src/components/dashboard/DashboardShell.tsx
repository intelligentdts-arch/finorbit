...
export default function DashboardShell({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  const navBtn = (item: { icon: string; label: string; path: string; badge?: string }) => (
    <button key={item.path} onClick={() => { router.push(item.path); setSidebarOpen(false) }}
      ...
      <span style={{ fontSize:"0.65rem", fontFamily:"DM Mono,monospace", width:20, textAlign:"center", opacity:.7 }}>{item.icon}</span>
      {item.label}
  ...
  return (
    <div style={{ minHeight:"100vh", background:S.black, display:"flex" }}>
      {sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", backdropFilter:"blur(4px)", zIndex:49 }}/>}
      ...
      <aside style={{ position:"fixed", top:0, left:0, bottom:0, width:240, zIndex:50, background:S.deep, borderRight:`1px solid ${S.b3}`, display:"flex", flexDirection:"column" }}>
        <div style={{ padding:"22px 22px 18px", fontSize:"1.2rem", fontWeight:800, borderBottom:`1px solid ${S.b3}`, background:"linear-gradient(135deg,#e0f2fe,#38bdf8,#2dd4bf)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>FinOrbit</div>
        <div style={{ margin:"8px", display:"flex", alignItems:"center", gap:6, padding:"8px 10px", background:"rgba(45,212,191,.08)", border:"1px solid rgba(45,212,191,.2)", borderRadius:8, fontSize:"0.7rem", fontFamily:"DM Mono,monospace", color:"#5eead4" }}>
          <span style={{ width:6, height:6, borderRadius:"50%", background:"#2dd4bf", flexShrink:0 }}/> Autopilot Active
        </div>
        ...
        <div style={{ marginTop:"auto", padding:"14px 12px", borderTop:`1px solid ${S.b3}` }}>
          <button onClick={handleSignOut} style={{ display:"flex", alignItems:"center", gap:10, padding:10, borderRadius:8, width:"100%", cursor:"pointer", background:"transparent", border:"none", fontFamily:"inherit" }}>
      ...
      <div style={{ flex:1, marginLeft:240, minWidth:0, display:"flex", flexDirection:"column" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 32px", borderBottom:`1px solid ${S.b3}`, background:"rgba(4,12,26,.9)", backdropFilter:"blur(12px)", position:"sticky", top:0, zIndex:40 }}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={() => router.push("/dashboard/opportunities")} style={{ padding:"8px 16px", borderRadius:8, fontSize:"0.78rem", fontWeight:800, background:S.grad, color:"#020d1a", border:"none", cursor:"pointer", fontFamily:"inherit" }}>3 Opportunities</button>
            <button onClick={handleSignOut} style={{ padding:"8px 14px", borderRadius:8, fontSize:"0.78rem", fontWeight:600, background:S.panel, color:S.muted, border:`1px solid ${S.b2}`, cursor:"pointer", fontFamily:"inherit" }}>Sign Out</button>
          </div>
        </div>
