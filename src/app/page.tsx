  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background: S.black }}>
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:'2rem', fontWeight:800, marginBottom:'1rem', ...S.gradText }}>FinOrbit</div>
        <div style={{ width:32, height:32, border:'2px solid rgba(34,211,238,0.2)', borderTopColor:'#22d3ee', borderRadius:'50%', animation:'spin .8s linear infinite', margin:'0 auto' }}/>
      </div>
    </div>
  )
    ...
    <>
      ...
      <nav style={{
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={() => openAuth('signin')} style={{ background:'transparent', color:S.text, padding:'9px 20px', borderRadius:5, fontSize:'0.82rem', fontWeight:700, border:`1px solid ${S.b1}`, cursor:'pointer', fontFamily:'inherit' }}>Sign In</button>
          <button onClick={() => openAuth('signup')} style={{ background:S.grad, color:'#020d1a', padding:'9px 22px', borderRadius:5, fontSize:'0.82rem', fontWeight:800, border:'none', cursor:'pointer', fontFamily:'inherit', boxShadow:'0 0 20px rgba(34,211,238,0.2)' }}>Get Started →</button>
        </div>
      </nav>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'230px 1fr 260px', minHeight:390 }}>
            ...
            <div style={{ padding:'24px 28px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:13, marginBottom:22 }}>
                {[{l:'Net Worth',v:'$284,940',c:S.gradText,ch:'↑ +12.4% this quarter'},{l:'Monthly Cash Flow',v:'$6,200',c:{color:'#5eead4'},ch:'↑ $480 from last month'},{l:'Autopilot Score',v:'94 / 100',c:{color:S.green},ch:'↑ +3 pts this week'}].map(k => (
              ...
              {[{l:'Survival (Bills)',v:'$1,860 · 30%',w:'30%',col:'linear-gradient(90deg,#c53030,#f87171)'},{l:'Stability (Savings)',v:'$1,240 · 20%',w:'20%',col:'linear-gradient(90deg,#1d4ed8,#60a5fa)'},{l:'Growth (Investments)',v:'$1,860 · 30%',w:'30%',col:'linear-gradient(90deg,#2dd4bf,#5eead4)'},{l:'Leverage + Opportunity',v:'$1,240 · 20%',w:'20%',col:`linear-gradient(90deg,${S.cyan},#67e8f9)`}].map(b => (
                <div key={b.l}>
                  <div style={{ fontSize:'0.7rem', color:S.muted, marginBottom:7, display:'flex', justifyContent:'space-between', fontFamily:'DM Mono,monospace' }}><span>{b.l}</span><span style={{ color:S.text }}>{b.v}</span></div>
                  <div style={{ height:5, borderRadius:10, background:S.b3, marginBottom:10, overflow:'hidden' }}><div style={{ width:b.w, height:'100%', borderRadius:10, background:b.col }}/></div>
                </div>
              ))}
            </div>
            {/* feed */}
            <div style={{ borderLeft:`1px solid ${S.b3}`, padding:'22px 16px' }}>
              ...
              {[{i:'📈',bg:'rgba(45,212,191,.1)',a:'Moved $340 to index fund',s:'2 hrs ago · auto-invested'},{i:'⚡',bg:'rgba(34,211,238,.1)',a:'Auto-paid mortgage & utilities',s:'9:00 AM · $2,140'},{i:'🎯',bg:'rgba(52,211,153,.1)',a:'HYSA opportunity detected',s:'Switching $4k → 5.1% APY'},{i:'🛡️',bg:'rgba(251,191,36,.1)',a:'Emergency fund target hit',s:'Reallocating surplus'},{i:'💳',bg:'rgba(45,212,191,.1)',a:'Refinance opportunity found',s:'Save $189/mo on auto loan'}].map(f => (
                <div key={f.a} style={{ display:'flex', gap:9, alignItems:'flex-start', padding:'10px 0', borderBottom:`1px solid ${S.b3}` }}>
                  ...
                  <div><div style={{ fontSize:'0.71rem', color:S.text, lineHeight:1.5 }}>{f.a}</div><div style={{ fontSize:'0.63rem', color:S.muted, fontFamily:'DM Mono,monospace', marginTop:2 }}>{f.s}</div></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="how" style={{ padding:'100px 40px', background:S.deep, borderTop:`1px solid ${S.b3}` }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <p style={{ color:S.muted, fontSize:'1rem', maxWidth:520, lineHeight:1.85, marginBottom:64 }}>FinOrbit doesn't give suggestions. It takes action. Here's the complete loop running in the background, every single day.</p>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:1, background:S.b3, border:`1px solid ${S.b1}`, borderRadius:14, overflow:'hidden' }}>
            {[{n:'STEP 01',i:'🔗',t:'Connect Everything',d:'Link your banks, investments, and credit via secure APIs. FinOrbit builds a complete financial picture in minutes.'},{n:'STEP 02',i:'🧠',t:'AI Learns You',d:'Reinforcement learning models analyze your income, spending, risk tolerance, and goals. Gets smarter daily.'},{n:'STEP 03',i:'⚙️',t:'Decide + Execute',d:'Autonomous AI agents pay bills, allocate capital, invest, and manage debt — context-aware, not rule-based.'},{n:'STEP 04',i:'📡',t:'Optimize Continuously',d:"Every action feeds back into the model. As markets shift and goals evolve, FinOrbit adapts in real time."}].map(s => (
        ...
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16 }}>
            {[
              ...
              {n:'MODULE 02', bg:'rgba(56,189,248,.1)', i:'💰', t:'Income Engine', d:'Detects, categorizes, and forecasts all income streams — salary, business revenue, rental, investment returns. Predicts your next payday and income volatility.', pills:['Plaid + Stripe','AI forecasting']},
              {n:'MODULE 03', bg:'rgba(45,212,191,.1)', i:'🤖', t:'Autonomous Execution', d:'AI agents that actually do things — pay bills, move money, invest in portfolios, refinance loans. Context-aware decisions, not rule-based.', pills:['Agentic AI','Full autopilot mode']},
              {n:'MODULE 04', bg:'rgba(34,211,238,.1)', i:'📊', t:'Predictive Intelligence', d:'Forecasts cash flow gaps, default risk, investment windows, and macro impacts — weeks before they happen.', pills:['Cash flow forecasting','Risk alerts']},
              {n:'MODULE 05', bg:'rgba(248,113,113,.08)', i:'💳', t:'Smart Borrowing Engine', d:'Turns debt into a strategic tool. AI decides when and how much to borrow to maximize ROI while minimizing interest cost.', pills:['ROI vs interest calc','Auto refinance']},
              {n:'MODULE 06', bg:'rgba(56,189,248,.1)', i:'📈', t:'Investment Autopilot', d:'Allocates across stocks, real estate, crypto, and business. Auto-rebalances and tax-optimizes. Factors in major life decisions.', pills:['Multi-asset','Tax-loss harvesting']},
              {n:'MODULE 07', bg:'rgba(45,212,191,.1)', i:'🧠', t:'Behavioral Layer', d:'Learns your spending psychology and adapts. If you overspend, the system tightens discretionary flow automatically.', pills:['Habit modeling','Behavioral nudges']},
              {n:'MODULE 08', bg:'rgba(248,113,113,.08)', i:'🛡️', t:'Risk + Protection Layer', d:'Fraud detection via anomaly models, automated emergency fund building, insurance gap analysis, and economic stress-testing.', pills:['AI fraud detection','Stress testing']},
            ].map((f,i) => (
        ...
        <div style={{ maxWidth:1100, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'center' }}>
          <div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:11 }}>
              {[{c:'#f87171',n:'Survival',s:'Bills, rent, essentials',p:'30%'},{c:'#60a5fa',n:'Stability',s:'Emergency fund, savings',p:'20%'},{c:'#2dd4bf',n:'Growth',s:'Investments, assets',p:'30%'},{c:S.cyan,n:'Leverage + Opportunity',s:'Strategic debt, high-ROI moves',p:'20%'}].map(l => (
                <div key={l.n} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 17px', background:S.panel, border:`1px solid ${S.b2}`, borderRadius:9 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:11 }}>
                    ...
                    <div><div style={{ fontSize:'0.82rem', fontWeight:600 }}>{l.n}</div><div style={{ fontSize:'0.66rem', color:S.muted, fontFamily:'DM Mono,monospace' }}>{l.s}</div></div>
                  </div>
      ...
      <section style={{ padding:'100px 40px', background:S.black }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:18 }}>
            {[{v:83,c:'#38bdf8',n:'Stability Score',d:'Measures your buffer against financial shocks — emergency fund, debt coverage ratio, and income diversification.',off:42},{v:90,c:'#2dd4bf',n:'Growth Score',d:'Tracks the velocity and efficiency of your wealth-building — investment returns, asset allocation quality, compounding rate.',off:25},{v:76,c:'#7dd3fc',n:'Risk Score',d:'Evaluates exposure to market risk, over-leverage, insurance gaps, and behavioral patterns that could create vulnerability.',off:60}].map(sc => (
