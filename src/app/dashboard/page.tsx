        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: S.panel, border: `1px solid ${S.b2}`, borderRadius: 14, padding: 22 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {[
                { label: 'up',  bg: 'rgba(45,212,191,.12)', action: 'Surplus allocated to Growth bucket',      meta: 'Just now',    impact: `+${fmt((data?.monthly_surplus || 0) * 0.5)} deployed`, up: true  },
                { label: 'al',  bg: 'rgba(34,211,238,.12)', action: 'Upcoming bills detected and reserved',    meta: '2 hours ago', impact: 'Zero late fees guaranteed',                           up: false },
                { label: 'op',  bg: 'rgba(52,211,153,.12)', action: 'Opportunity identified in your accounts', meta: 'Yesterday',   impact: 'View in Opportunities',                               up: true  },
                { label: 'an',  bg: 'rgba(251,191,36,.1)',  action: 'Spending pattern analyzed',               meta: '2 days ago',  impact: 'Budget adjusted automatically',                       up: false },
              ].map((f, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, padding: '11px 8px', borderBottom: `1px solid ${S.b3}` }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', fontFamily: 'DM Mono,monospace', color: S.muted, flexShrink: 0, background: f.bg }}>{f.label}</div>
                  <div>
