...
// Plaid: positive amount = money going OUT
...
export const calculateMonthlySurplus = (income: number, expenses: number): number =>
  roundMoney(income - expenses)
...
// Dynamic allocation based on risk profile
export const calculateAllocationBuckets = (
  ...
  const profiles = {
    conservative: { survival: 0.40, stability: 0.30, growth: 0.20, leverage: 0.05, opportunity: 0.05 },
    balanced:     { survival: 0.30, stability: 0.20, growth: 0.30, leverage: 0.12, opportunity: 0.08 },
    aggressive:   { survival: 0.25, stability: 0.10, growth: 0.40, leverage: 0.15, opportunity: 0.10 },
  }
...
// Project net worth N months from now
export const projectNetWorth = (
  ...
  for (let i = 0; i < months; i++) {
    projected = projected * (1 + r) + monthlySurplus
  }
  return roundMoney(projected)
}

// Autopilot score from real financial indicators
export const calculateAutopilotScore = (params: {
  ...
  const stability = Math.min(100, Math.round(
    (params.hasEmergencyFund ? 40 : 0) +
    Math.min(30, params.savingsRate * 1.5) +
    (params.creditUtilization < 30 ? 30 : params.creditUtilization < 50 ? 15 : 0)
  ))

  const growth = Math.min(100, Math.round(
    (params.hasInvestments ? 40 : 0) +
    Math.min(40, params.savingsRate * 2) +
    (params.savingsRate > 20 ? 20 : params.savingsRate > 10 ? 10 : 0)
  ))

  const risk = Math.min(100, Math.round(
    (params.debtToIncomeRatio < 0.36 ? 50 : params.debtToIncomeRatio < 0.5 ? 25 : 0) +
    (params.creditUtilization < 10 ? 30 : params.creditUtilization < 30 ? 20 : 10) +
    (params.hasEmergencyFund ? 20 : 0)
  ))
